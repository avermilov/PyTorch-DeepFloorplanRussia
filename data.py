...
def dynamic_crop(image, boundary, room):
    ...
    while True:
        if np.unique(
                np.concatenate([image[0, :, :].reshape(-1, 3), image[-1, :, :].reshape(-1, 3),
                                image[:, 0, :].reshape(-1, 3), image[:, -1, :].reshape(-1, 3)]), axis=0
        ).shape[0] == 1:
            image = image[1:-1, 1:-1, :]
    ...
    while True:
        if h > w:
            if np.unique(np.concatenate([image[0, :, :].reshape(-1, 3),
                                         image[-1, :, :].reshape(-1, 3)]), axis=0).shape[0] == 1:
                image = image[1:-1, :, :]
...
def localize(image, boundary, room, pixel_mean_thr: int = 240):
    while True:
        perimeter = np.concatenate([image[0, :, :].reshape(-1, 3), image[-1, :, :].reshape(-1, 3)])
...
class FillShape(nn.Module):
    ...
    def forward(self, image, boundary, room):
        ...
        if h > w:
            ...
            pad_w_left = (h - w) // 2 if self.type == "center" else np.random.randint(0, h - w + 1)
            pad_w_right = h - w - pad_w_left
        else:
            pad_w_left, pad_w_right = 0, 0
            pad_h_left = (w - h) // 2 if self.type == "center" else np.random.randint(0, w - h + 1)
...
class r3dDataset(Dataset):
    def __init__(self, root_dir: str, num_boundary: int, num_room: int,
                 ...
                 fill_type: str = "none", crop_type: str = "none"):
