...
def getloader(args):
    ...
    # trans = A.Compose([A.HorizontalFlip(), A.VerticalFlip(), ])
    r3d = r3dDataset("r3d.csv", transform=trans)
    total_image = len(r3d)
    ...
    train_set, val_set = torch.utils.data.random_split(r3d, [len(r3d) - split, split])
...
def main(args):
    ...
    for epoch in range(args.maxiters):
        ...
        for idx, (im, cw, r, _) in enumerate(train_loader):
            ...
            if idx % 20 == 0:
                print('[INFO] Batch {}/{:d} Train Loss: {:.1f}, R Loss: {:.1f}, B Loss: {:.1f}'.format(
                    idx, int(total_batch * (1 - args.valsplit)), loss.item(), loss1.item(), loss2.item()))
        ...
        if args.earlystop:
            ...
            torch.save(model.state_dict(), 'log/store10/checkpoint.pt')
